...
def test_model(model_path, batch_size=64, save_file=False, test_path=None):
    ...
    for testSet in DetectionTests.keys():
        ...
        for v_id, val in enumerate(sorted(os.listdir(dataroot))):
            ...
            test_results['subsets'][val] = metrics
            acc, ap, f1, roc = metrics['acc'], metrics['ap'], metrics['f1'], metrics['auc_roc']
            accs.append(acc)
            aps.append(ap)
            f1s.append(f1)
            rocs.append(roc)
        ...
        mean_metrics = {
            'acc': float(np.array(accs).mean()),
            'ap': float(np.array(aps).mean()),
            'f1': float(np.array(f1s).mean()),
            'auc_roc': float(np.array(rocs).mean()),
        }
        test_results['mean'] = mean_metrics

        print(
            f"(Mean    ) mAcc: {mean_metrics['acc'] * 100:5.2f}, "
            f"mAP: {mean_metrics['ap'] * 100:5.2f}, "
            f"mF1: {mean_metrics['f1'] * 100:5.2f}, "
            f"mAuc_Roc: {mean_metrics['auc_roc'] * 100:5.2f}"
        )
        print('*' * 50)
        ...
        if opt.save_file:
            ...
            save_results_json(test_results, json_filename)

        all_results[testSet] = test_results
    ...
    if len(DetectionTests.keys()) > 1:
        ...
        for testSet, results in all_results.items():
            mean = results['mean']
            print(f"{testSet:20} - Acc: {mean['acc'] * 100:5.2f}, AP: {mean['ap'] * 100:5.2f}")
        print('=' * 50)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_file', action='store_true', help='Save test results to JSON')
